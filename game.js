...
function buildPropSpatialGrid(){
  ...
  // Also inject the active zone's landmark as a collidable
...
function drawEnvironment(now){
  ...
  // Render the active zone's landmark sprite if one exists and is in view
...
// ═══════ SPRITE PRELOAD ═════════════════════════════════════
// Kick off sprite loading in the background. Sprites load while the user is on
// the title screen. If the game starts before sprites finish loading, that's
// fine — drawSpriteProp falls back to labeled placeholders, and the real
// sprites slot in as they finish loading.
loadSprites().then(() => {
...
function drawSpriteProp(key, x, y, scale = 1, rotation = 0){
  ...
  ctx.translate(x, y);
  if (rotation) ctx.rotate(rotation);
  // Draw cast shadow first — soft ellipse beneath the sprite.
  // Shadow size scales with sprite and shifts slightly right for "sun from top-left" feel.
  const w = img.width * scale;
  const h = img.height * scale;
  ctx.fillStyle = 'rgba(0,0,0,0.4)';
  ctx.beginPath();
  ctx.ellipse(w * 0.05, h * 0.4, w * 0.35, h * 0.09, 0, 0, Math.PI * 2);
  ctx.fill();
  // Draw the actual sprite centered around (x,y). The sprite's "anchor" is
  // bottom-center — the point that "touches the ground." So we shift up by h/2.
  ctx.drawImage(img, -w/2, -h + h*0.15, w, h);
  ctx.restore();
}
