...
// ═══════════════════════════════════════════════════════════════════
// ZONE_NPCS — Quest-giver NPCs scattered throughout the overworld zones.
// Each zone gets 2 NPCs at fixed world coordinates. Players walk up to
// them and interact (E key / tap) to get zone-specific quests.
//
// Think WoW Classic — finding an NPC in the field who offers you work
// suited to that zone's enemies and level range.
// ═══════════════════════════════════════════════════════════════════
const ZONE_NPCS = [
  // ─── ASHEN WASTES (zone id: 'ashen') — levels 1-10 ───
  {
    id: 'ashen_mourner',
    name: 'The Ashen Mourner',
    zoneId: 'ashen',
    x: 2200, y: 1800,                   // northwest-ish
    color: '#8a7ea8', accent: '#c4b5fd',
    description: 'She kneels beside a cairn of skulls and counts them, over and over.',
    flavor: '"There were more. There were more. Find the ones that wandered."',
    npcType: 'ghost-scholar',
  },
  {
    id: 'ashen_scout',
    name: 'The Broken Scout',
    zoneId: 'ashen',
    x: 4500, y: 4100,                   // southeast
    color: '#c86a32', accent: '#f08a4a',
    description: 'A ranger who walked too far into the wastes and never left.',
    flavor: '"I marked their paths before I died. The crawlers come in waves. Help me cull them."',
    npcType: 'ghost-warrior',
  },

  // ─── BONE CRYPTS (zone id: 'crypts') — levels 8-22 ───
  {
    id: 'crypts_keeper',
    name: 'The Crypt-Keeper',
    zoneId: 'crypts',
    x: 2000, y: 2300,
    color: '#d4a04a', accent: '#fde68a',
    description: 'He guards the doors that no one has tried to open in centuries.',
    flavor: '"The wraiths grow bold. Their numbers thicken. I require evidence of their thinning."',
    npcType: 'ghost-warrior',
  },
  {
    id: 'crypts_scribe',
    name: 'The Last Scribe',
    zoneId: 'crypts',
    x: 4700, y: 3900,
    color: '#9DC4B0', accent: '#b8e0c8',
    description: 'She was writing a history of the Veil when it collapsed. She has not stopped writing.',
    flavor: '"Bring me the names of the fallen elites. I need them before the ink runs dry."',
    npcType: 'ghost-scholar',
  },

  // ─── EMBER BARRENS (zone id: 'ember') — levels 20-40 ───
  {
    id: 'ember_warden',
    name: 'The Ember Warden',
    zoneId: 'ember',
    x: 2400, y: 2100,
    color: '#ef4444', accent: '#fbbf24',
    description: 'He stands in the hottest part of the barrens, and feels nothing.',
    flavor: '"The deeper fire still burns. Clear the gate and I will remember your name."',
    npcType: 'ghost-warrior',
  },
  {
    id: 'ember_pilgrim',
    ...
    description: 'She walks in circles — the same circles, forever — praying to nothing.',
    flavor: '"The bosses of the deep places know my name. Slay one. Silence them."',
    npcType: 'ghost-scholar',
  },
];
...
const XP_ACTIVITY_MULT = {
  ...
  // Quest XP rewards — cut in half so there's room for a larger quest catalog
  // without trivializing leveling. Players complete MORE quests at LESS xp each
  // so the story content feels like an ongoing thread, not a few big payoffs.
  smallQuest:  5,
  stdQuest:    10,
  majorQuest:  20,
  storyQuest:  50,
...
const TALENT_TREE={
  Binding:{
    ...
    talents:[
      ...
      // ─── DEEP BINDING (gate 8+) ─────────────────────────────────
      {id:'b7',name:'Iron Bond',icon:'◉',maxRank:3,gate:8,
       desc:'Spirits have +15% HP per rank, surviving longer in combat.',
       effect:r=>`+${r*15}% spirit HP`,
       apply:r=>({spiritHpPct:r*15})},
      {id:'b8',name:'Sanguine Pact',icon:'♡',maxRank:3,gate:10,
       desc:'Spirits heal you for 2% of their damage dealt per rank.',
       effect:r=>`Spirits heal ${r*2}% of damage to you`,
       apply:r=>({spiritLifestealPct:r*2})},
      {id:'b9',name:'Greater Summoning',icon:'◈',maxRank:1,gate:14,
       desc:'Raise always summons an additional spirit (stacks with Echoing Call).',
       effect:_=>'Raise +1 extra spirit',
       apply:_=>({raisePlusOne:1})},
      {id:'b10',name:'Procession of the Dead',icon:'✪',maxRank:3,gate:18,
       desc:'Each living spirit grants you +2% damage reduction per rank.',
       effect:r=>`-${r*2}% dmg taken per spirit`,
       apply:r=>({perSpiritDrPct:r*2})},
      {id:'b11',name:'Dominion',icon:'⚒',maxRank:3,gate:22,
       desc:'Spirit count cap increased further.',
       effect:r=>`+${r} spirit cap`,
       apply:r=>({spiritCap:r})},
      {id:'b12',name:'Legion Commander',icon:'★',maxRank:1,gate:26,
       desc:'While 8+ spirits are alive, gain +50% damage and 25% DR.',
       effect:_=>'8+ spirits: +50% dmg, +25% DR',
       apply:_=>({legionMode:1})},
    ],
  },
  Veilcraft:{
    classId:'hollowcaller',
    ...
    talents:[
      ...
      {id:'v6',name:'Cataclysm',icon:'✹',maxRank:1,gate:6,
       ...
       apply:_=>({detoEcho:1})},
      // ─── DEEP VEILCRAFT (gate 8+) ───────────────────────────────
      {id:'v7',name:'Bleeding Veil',icon:'◈',maxRank:3,gate:8,
       desc:'Veilmarks linger 2s longer per rank before expiring.',
       effect:r=>`+${r*2}s Veilmark duration`,
       apply:r=>({veilmarkDurationMs:r*2000})},
      {id:'v8',name:'Searing Eruption',icon:'☀',maxRank:3,gate:10,
       desc:'All AOE abilities deal +5% damage per rank.',
       effect:r=>`+${r*5}% AOE damage`,
       apply:r=>({aoeDmgPct:r*5})},
      {id:'v9',name:'Wounding Mark',icon:'⊗',maxRank:3,gate:14,
       desc:'Veilmarked enemies take +3% damage per stack from all sources.',
       effect:r=>`Marks = +${r*3}% dmg taken per stack`,
       apply:r=>({veilmarkVulnPct:r*3})},
      {id:'v10',name:'Sundering Veil',icon:'⚡',maxRank:3,gate:18,
       desc:'All ability cooldowns reduced further.',
       effect:r=>`-${r*5}% all cooldowns`,
       apply:r=>({cdrPct:r*5})},
      {id:'v11',name:'Endless Harvest',icon:'✪',maxRank:2,gate:22,
       desc:'Detonate has +20% chance per rank to echo (stacks with Cataclysm).',
       effect:r=>`+${r*20}% Detonate echo chance`,
       apply:r=>({detoEchoChance:r*20})},
      {id:'v12',name:'Final Eruption',icon:'★',maxRank:1,gate:26,
       desc:'Detonate radius tripled. Damage halved but hits all enemies in zone.',
       effect:_=>'Detonate: 3x radius, 0.5x damage',
       apply:_=>({finalEruption:1})},
    ],
  },
  Hollow:{
    classId:'hollowcaller',
    ...
    talents:[
      ...
      // ─── DEEP HOLLOW (gate 8+) ──────────────────────────────────
      {id:'h7',name:'Hollow Body',icon:'♡',maxRank:3,gate:8,
       desc:'Max HP increased further.',
       effect:r=>`+${r*6}% max HP`,
       apply:r=>({hpPct:r*6})},
      {id:'h8',name:'Drifting Step',icon:'↦',maxRank:3,gate:10,
       desc:'Move speed further increased.',
       effect:r=>`+${r*3}% movement speed`,
       apply:r=>({moveSpdPct:r*3})},
      {id:'h9',name:'Pale Continuance',icon:'❂',maxRank:3,gate:14,
       desc:'Heal more HP on each enemy death.',
       effect:r=>`Heal ${r*8} HP per kill`,
       apply:r=>({lifeOnHit:r*8})},
      {id:'h10',name:'Unerring Hand',icon:'◇',maxRank:3,gate:18,
       desc:'Crits deal +20% damage per rank.',
       effect:r=>`+${r*20}% crit damage`,
       apply:r=>({critDmgPct:r*20})},
      {id:'h11',name:'Veilwalker',icon:'⊠',maxRank:3,gate:22,
       desc:'Further damage reduction.',
       effect:r=>`-${r*4}% damage taken`,
       apply:r=>({dmgReducePct:r*4})},
      {id:'h12',name:'The Still Heart',icon:'★',maxRank:1,gate:26,
       desc:'Cheat death triggers twice per life instead of once.',
       effect:_=>'Cheat death triggers 2x',
       apply:_=>({cheatDeath:1,cheatDeathCount:2})},
    ],
  },
  ...
  Ironclad:{
    ...
    talents:[
      ...
      {id:'i6',name:'Unyielding',icon:'✦',maxRank:1,gate:6,
       ...
       apply:_=>({cheatDeath:1})},
      // ─── DEEP IRONCLAD (gate 8+) ────────────────────────────────
      {id:'i7',name:'Ironbound Flesh',icon:'♡',maxRank:3,gate:8,
       desc:'Max HP further increased.',
       effect:r=>`+${r*8}% max HP`,
       apply:r=>({hpPct:r*8})},
      {id:'i8',name:'Implacable',icon:'⇵',maxRank:3,gate:10,
       desc:'Cannot be slowed or rooted below 30% duration per rank.',
       effect:r=>`-${r*25}% CC duration`,
       apply:r=>({ccReducePct:r*25})},
      {id:'i9',name:'Riposte',icon:'↯',maxRank:3,gate:14,
       desc:'Blocked attacks deal reflected damage back to attacker.',
       effect:r=>`Blocks reflect ${r*40}% damage`,
       apply:r=>({blockReflectPct:r*40})},
      {id:'i10',name:'Heart of Stone',icon:'⊠',maxRank:3,gate:18,
       desc:'Further damage reduction.',
       effect:r=>`-${r*5}% damage taken`,
       apply:r=>({dmgReducePct:r*5})},
      {id:'i11',name:'Unbreakable Stance',icon:'⬟',maxRank:3,gate:22,
       desc:'Block chance further increased.',
       effect:r=>`${r*5}% chance to fully block`,
       apply:r=>({blockChance:r*5})},
      {id:'i12',name:'Immortal',icon:'★',maxRank:1,gate:26,
       desc:'Cheat death triggers twice per life. When it triggers, heal to 50%.',
       effect:_=>'Cheat death x2, heal to 50%',
       apply:_=>({cheatDeath:1,cheatDeathCount:2,cheatDeathHealPct:50})},
    ],
  },
  ...
  Warborn:{
    ...
    talents:[
      ...
      // ─── DEEP WARBORN (gate 8+) ─────────────────────────────────
      {id:'w7',name:'Steel Edge',icon:'▲',maxRank:3,gate:8,
       desc:'Further melee damage increase.',
       effect:r=>`+${r*6}% melee damage`,
       apply:r=>({meleeDmgPct:r*6})},
      {id:'w8',name:'Devastation',icon:'※',maxRank:3,gate:10,
       desc:'Cleaved damage increased per rank.',
       effect:r=>`+${r*25}% cleave damage`,
       apply:r=>({cleaveDmgPct:r*25})},
      {id:'w9',name:'Executioner\'s Resolve',icon:'◇',maxRank:3,gate:14,
       desc:'Bonus crit chance vs wounded enemies (below 50% HP).',
       effect:r=>`+${r*10}% crit vs low HP`,
       apply:r=>({lowHpCritPct:r*10})},
      {id:'w10',name:'Perfect Form',icon:'↯',maxRank:2,gate:18,
       desc:'Juggernaut momentum stacks decay even slower.',
       effect:r=>`Momentum +${r*2}s duration`,
       apply:r=>({momentumDecayBonus:r*2000})},
      {id:'w11',name:'Cleaving Storm',icon:'✖',maxRank:3,gate:22,
       desc:'Further damage vs wounded enemies.',
       effect:r=>`+${r*12}% dmg vs wounded`,
       apply:r=>({executeDmgPct:r*12})},
      {id:'w12',name:'The Unending Blade',icon:'★',maxRank:1,gate:26,
       desc:'All kills (not just elites) have 25% chance to refresh a random CD.',
       effect:_=>'25% CD refresh on any kill',
       apply:_=>({anyKillRefreshChance:25})},
    ],
  },
  ...
  Bloodbound:{
    ...
    talents:[
      ...
      // ─── DEEP BLOODBOUND (gate 8+) ──────────────────────────────
      {id:'bl7',name:'Deep Thirst',icon:'♡',maxRank:3,gate:8,
       desc:'Further lifesteal per rank.',
       effect:r=>`${r*2}% lifesteal`,
       apply:r=>({lifestealPct:r*2})},
      {id:'bl8',name:'Offering Accepted',icon:'▼',maxRank:3,gate:10,
       desc:'Further damage when below 50% HP.',
       effect:r=>`+${r*6}% damage below 50% HP`,
       apply:r=>({painOfferingPct:r*6})},
      {id:'bl9',name:'Reckless Strike',icon:'⊗',maxRank:2,gate:14,
       desc:'First hit after cast deals massive bonus damage.',
       effect:r=>`+${r*30}% first-hit damage`,
       apply:r=>({bloodPricePct:r*30})},
      {id:'bl10',name:'Relentless Ravage',icon:'↻',maxRank:3,gate:18,
       desc:'Further cooldown refund on kill.',
       effect:r=>`Kills refund ${r*3}% CD`,
       apply:r=>({ravageCdrPct:r*3})},
      {id:'bl11',name:'Savage Joy',icon:'✹',maxRank:1,gate:22,
       desc:'Every 3rd hit deals +100% damage (was every 5th).',
       effect:_=>'Every 3rd hit +100% dmg',
       apply:_=>({ruinousStrike:1,ruinousStrikeEvery:3})},
      {id:'bl12',name:'Crimson Sovereign',icon:'★',maxRank:1,gate:26,
       desc:'Crimson Ascendance triggers twice per fight.',
       effect:_=>'Crimson Ascendance x2',
       apply:_=>({crimsonAscendance:1,crimsonAscendanceCount:2})},
    ],
  },
};
