...
function checkVeilgateUnlock(){
  if(!veilgateState.unlocked && player.level >= VEILGATE_UNLOCK_LEVEL){
    ...
    // Big visual announcement banner
    if(typeof showVeilgateUnlockBanner === 'function') showVeilgateUnlockBanner();
    if(typeof writeSave === 'function') writeSave();
  }
  // Always refresh menu button visibility (handles reload case)
  refreshVeilgateMenuVisibility();
  return veilgateState.unlocked;
}

// Shows/hides the Veilgate menu button based on unlock state.
// Called on boot and after each level-up.
function refreshVeilgateMenuVisibility(){
  const btn = document.getElementById('veilgateMenuBtn');
  if(!btn) return;
  btn.style.display = veilgateState.unlocked ? '' : 'none';
}

// One-time celebration banner when Veilgate first unlocks
function showVeilgateUnlockBanner(){
  const banner = document.createElement('div');
  banner.id = 'veilgateUnlockBanner';
  banner.innerHTML = `
    <div class="vg-banner-inner">
      <div class="vg-banner-title">⚡ THE VEILGATE HAS OPENED ⚡</div>
      <div class="vg-banner-sub">The Veil trembles. A new path reveals itself.</div>
      <div class="vg-banner-hint">Seek the Veilwarden in camp.</div>
    </div>
  `;
  document.body.appendChild(banner);
  setTimeout(()=>{
    banner.classList.add('vg-banner-fade');
...
if(typeof window !== 'undefined'){
  ...
  window.refreshVeilgateMenuVisibility = refreshVeilgateMenuVisibility;
  window.showVeilgateUnlockBanner = showVeilgateUnlockBanner;
